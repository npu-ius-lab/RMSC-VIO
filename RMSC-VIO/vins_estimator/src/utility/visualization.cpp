...
ros::Publisher pub_point_cloud, pub_margin_cloud, pub_point_cloud1, pub_margin_cloud1;
ros::Publisher pub_key_poses;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose_visual;

ros::Publisher pub_camera_pose1;
ros::Publisher pub_camera_pose1_visual;
...
ros::Publisher pub_extrinsic1;

ros::Publisher pub_image_track;
ros::Publisher pub_image_track1;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_path = n.advertise<nav_msgs::Path>("path1", 1000);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 1000);
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", 1000);

    pub_point_cloud1 = n.advertise<sensor_msgs::PointCloud>("point1_cloud", 1000);

    pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>("margin_cloud", 1000);
    pub_margin_cloud1 = n.advertise<sensor_msgs::PointCloud>("margin_cloud1", 1000);

    pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", 1000);
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("camera_pose", 1000);
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 1000);

    pub_camera_pose1 = n.advertise<nav_msgs::Odometry>("camera1_pose", 1000);
    pub_camera_pose1_visual = n.advertise<visualization_msgs::MarkerArray>("camera1_pose_visual", 1000);

    pub_keyframe_pose = n.advertise<nav_msgs::Odometry>("keyframe_pose", 1000);
    pub_keyframe_point = n.advertise<sensor_msgs::PointCloud>("keyframe_point", 1000);
    pub_extrinsic = n.advertise<nav_msgs::Odometry>("extrinsic", 1000);
    pub_extrinsic1 = n.advertise<nav_msgs::Odometry>("extrinsic1", 1000);
    pub_image_track = n.advertise<sensor_msgs::Image>("image_track_camera0", 1000);
    pub_image_track1 = n.advertise<sensor_msgs::Image>("image_track_camera1", 1000);

    cameraposevisual.setScale(0.1);
    cameraposevisual.setLineWidth(0.01);

    cameraposevisual1.setScale(0.1);
    cameraposevisual1.setLineWidth(0.01);
}
...
void pubTrackImage1(const cv::Mat &imgTrack, const double t)
{
    std_msgs::Header header;
    header.frame_id = "world";
    header.stamp = ros::Time(t);
...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    if (ESTIMATE_EXTRINSIC1)
    {
        cv::FileStorage fs(EX_CALIB_RESULT_PATH1, cv::FileStorage::WRITE);
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            //ROS_DEBUG("calibration result for camera %d", i);
            ROS_DEBUG_STREAM("extirnsic tic: " << estimator.tic1[i].transpose());
            ROS_DEBUG_STREAM("extrinsic ric: " << Utility::R2ypr(estimator.ric1[i]).transpose());

            Eigen::Matrix4d eigen_T = Eigen::Matrix4d::Identity();
            eigen_T.block<3, 3>(0, 0) = estimator.ric1[i];
            eigen_T.block<3, 1>(0, 3) = estimator.tic1[i];
            cv::Mat cv_T;
            cv::eigen2cv(eigen_T, cv_T);
            if(i == 0)
                fs << "body_T_cam0" << cv_T ;
...
void pubCameraPose1(const Estimator &estimator, const std_msgs::Header &header)
{
    int idx2 = WINDOW_SIZE - 1;
    ...
    {
        int i = idx2;
        Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic1[0];
        Quaterniond R = Quaterniond(estimator.Rs[i] * estimator.ric1[0]);
        ...
        cameraposevisual1.add_pose(P, R);
        if(STEREO)
        {
            Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic1[1];
            Quaterniond R = Quaterniond(estimator.Rs[i] * estimator.ric1[1]);
            cameraposevisual1.add_pose(P, R);
        }
        cameraposevisual1.publish_by(pub_camera_pose1_visual, odometry.header);
    }
}
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // {
        for (auto &it_per_id : estimator.f_manager.feature[0])
        {
            int used_num;
...
void pubPointCloud1(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // for(int c=0; c<NUM_OF_CAM; c++)
    // {
        for (auto &it_per_id : estimator.f_manager.feature[1])
        {
            int used_num;
            used_num = it_per_id.feature_per_frame.size();
            if (!(used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
            ...
            int imu_i = it_per_id.start_frame;
            Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
            Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric1[0] * pts_i + estimator.tic1[0]) + estimator.Ps[imu_i];
...
/************************************************************************************************/
    ...
    odometry.header = header;
    odometry.header.frame_id = "world";
    odometry.pose.pose.position.x = estimator.tic[1].x();
    odometry.pose.pose.position.y = estimator.tic[1].y();
    odometry.pose.pose.position.z = estimator.tic[1].z();
    Quaterniond tmp_q1{estimator.ric[1]};
    odometry.pose.pose.orientation.x = tmp_q1.x();
    odometry.pose.pose.orientation.y = tmp_q1.y();
    odometry.pose.pose.orientation.z = tmp_q1.z();
    odometry.pose.pose.orientation.w = tmp_q1.w();
    pub_extrinsic1.publish(odometry);
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        for (auto &it_per_id : estimator.f_manager.feature[1])
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                int imu_i = it_per_id.start_frame;
                Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
                Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric1[0] * pts_i + estimator.tic1[0])
                                      + estimator.Ps[imu_i];
                geometry_msgs::Point32 p;
                p.x = w_pts_i(0);
                p.y = w_pts_i(1);
                ...
                p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].point.y());
                p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].uv.x());
                p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].uv.y() + 480);
                p_2d.values.push_back(it_per_id.feature_id + feature_id);
                point_cloud.channels.push_back(p_2d);
                feature_id++;
            }
        }
