...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, cv::Mat &_image,cv::Mat &_image1,
		           ...
		           vector<double> &_point_id, int _sequence)
{
	...
	image = _image.clone();
	image1 = _image1.clone();
	cv::resize(image, thumbnail, cv::Size(160, 60));
	point_3d = _point_3d;
...
{
	// cout<<"111111111111111Cin KeyFrame::computeWindowBRIEFPoint"<<endl;
	...
	extractor(image1, window_keypoints, window_brief_descriptors);
}
...
void KeyFrame::computeBRIEFPoint()
{
	...
	if(1)
	{
		...
		cv::FAST(image1, keypoints, fast_th, true);
	}
	else
	{
		vector<cv::Point2f> tmp_pts, tmp_pts1;
		cv::goodFeaturesToTrack(image, tmp_pts, 500, 0.01, 10);
		cv::goodFeaturesToTrack(image1, tmp_pts1, 500, 0.01, 10);
		...
		for(int i = 0; i < (int)tmp_pts1.size(); i++)
		{
		    cv::KeyPoint key1;
		    key1.pt = tmp_pts1[i];
		    keypoints.push_back(key1);
		}
	}
	extractor(image, keypoints, brief_descriptors);
	extractor(image1, keypoints, brief_descriptors);
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	        ...
	        {
	        	int gap = 10;
	        	cv::Mat gap_image(2*ROW, gap, CV_8UC1, cv::Scalar(255, 255, 255));
	            cv::Mat gray_img, loop_match_img;
	            cv::Mat old_img, cur_image;
				cv::vconcat(old_kf->image, old_kf->image1, old_img);
				cv::vconcat(image, image1, cur_image);
				//hconcat水平拼接
	            cv::hconcat(cur_image, gap_image, gap_image); //当前帧与gap水平拼接为gap_image
	            cv::hconcat(gap_image, old_img, gray_img);//gap_image 与 old_img 拼接
	            cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);//灰度图转化为rgb图loop_match_img
	            for(int i = 0; i< (int)matched_2d_cur.size(); i++)
	            {
	                cv::Point2f cur_pt = matched_2d_cur[i];
	                cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 255, 0));
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    if ((abs(relative_yaw) < 30.0 || abs(relative_yaw - 180) < 30.0) && relative_t.norm() < 10.0)
	    {
